...
function UserButtonMenu() {
  ...
  const [userSwitchOpened, setUserSwitchOpened] = useState(false);
  ...
  const handleUserSwitch = (userType) => {
    switchUser(userType);
    setUserSwitchOpened(false);
    notifications.show({
      title: "Chuyển đổi thành công",
      message: `Đã chuyển sang tài khoản ${userType}`,
      color: "blue",
    });
  };

  const testUsers = [
    { value: "admin", label: "Admin - Quản trị viên", role: "admin", dept: "admin" },
    { value: "director", label: "Director - Giám đốc", role: "director", dept: "admin" },
    { value: "hr_manager", label: "HR Manager - Trưởng phòng Nhân sự", role: "hr", dept: "hr" },
    { value: "accountant", label: "Accountant - Trưởng phòng Kế toán", role: "manager", dept: "accounting" },
    { value: "employee", label: "Employee - Nhân viên kinh doanh", role: "employee", dept: "sales" },
  ];
  ...
  return (
    <>
      <Menu withArrow>
        ...
        <Menu.Dropdown>
          ...
          <Menu.Item
            ...
            onClick={() => setUserSwitchOpened(true)}
            c="blue"
          >
      ...
      {/* User Switch Modal (Demo) */}
      <Modal
        opened={userSwitchOpened}
        onClose={() => setUserSwitchOpened(false)}
        title="Chuyển đổi tài khoản (Demo)"
        centered
      >
        <Stack gap="md">
          <Alert icon={<IconInfoCircle size={16} />} color="yellow">
            Chức năng này chỉ dành cho demo và testing. Trong production, người dùng sẽ đăng nhập bằng tài khoản riêng.
          </Alert>

          <Text size="sm" c="dimmed">
            Hiện tại: <strong>{currentUser?.fullName}</strong> ({getRoleLabel(currentUser?.role)})
          </Text>
          ...
          <Stack gap="xs">
            {testUsers.map(user => (
              <Button
                key={user.value}
                variant={currentUser?.username === user.value ? "filled" : "outline"}
                onClick={() => handleUserSwitch(user.value)}
                disabled={currentUser?.username === user.value}
          ...
          <Button variant="outline" onClick={() => setUserSwitchOpened(false)}>
            Đóng
          </Button>
        </Stack>
      </Modal>
