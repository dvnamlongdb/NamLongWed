...
export default function UserStats({ userAccounts = [] }) {
  ...
  const stats = useMemo(() => {
    ...
    const pending = userAccounts.filter(u => u.status === "pending").length;
    const approved = userAccounts.filter(u => u.status === "approved").length;
    const active = userAccounts.filter(u => u.status === "active").length;
    const rejected = userAccounts.filter(u => u.status === "rejected").length;
    const inactive = userAccounts.filter(u => u.status === "inactive").length;
    ...
    const recentRequests = userAccounts.filter(user => 
      new Date(user.requestDate) >= thirtyDaysAgo
    ).length;
